...
function setup({ stored = {}, privacy = {}, deferLoaded = false, readFails = false } = {}) {
  ...
  const calls = { init: 0, stop: 0, optOut: 0, events: [] };
  const sdk = {
  ...
  const context = vm.createContext({
    ...
    process: { env: { NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN: "test-project", NEXT_PUBLIC_POSTHOG_HOST: "https://us.i.posthog.com" } },
    URL,
    require(name) { assert.equal(name, "posthog-js"); return { __esModule: true, default: sdk }; },
  });
  ...
  return { api: exports, sdk, calls, storage, window, navigator, updateFromOtherTab, failWrites() { writeFails = true; } };
}

const edited = { tool: "pencil", edit_type: "stroke", input_method: "pointer", linked_edges: false };

test("required-only and undecided visitors never initialize analytics or backfill pre-consent events", () => {
...
test("usage permits sanitized product events and page visits without replay or automatic diagnostics", () => {
...
test("enhanced diagnostics enable masked replay and redact error contents; downgrades stop them immediately", () => {
  const { api, sdk, calls } = setup();
...
test("legacy permission migrates to usage only and malformed new preferences cannot resurrect it", () => {
  for (const [legacy, expected] of [["granted", "usage"], ["denied", "required"]]) {
    const { api, sdk } = setup({ stored: { [legacyKey]: legacy } });
    api.initializeAnalytics();
...
test("a failed preference write disables analytics for the visit even over a previously saved opt-in", () => {
...
test("browser privacy signals override saved and newly requested optional levels", () => {
...
test("preferences synchronize across tabs and clearing stored consent stops an active recorder", () => {
...
test("a delayed SDK load cannot re-enable capture after withdrawal", () => {
