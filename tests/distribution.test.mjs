...
async function fixture(t){const root=await mkdtemp(join(tmpdir(),'pixelwall-package-'));t.after(()=>rm(root,{recursive:true,force:true}));for(const dir of ['scripts','dist/standalone','dist/cli'])await mkdir(join(root,dir),{recursive:true});await writeFile(join(root,'scripts/serve-standalone.mjs'),serverSource);await writeFile(join(root,'dist/standalone/index.html'),'<h1>PixelWall</h1>');await writeFile(join(root,'dist/cli/pixelwall.mjs'),'console.log("PixelWall CLI")');await writeFile(join(root,'dist/cli/package.json'),'{}');await writeFile(join(root,'dist/cli/README.md'),'PixelWall CLI');return root;}
...
test('both archives contain runnable expected entries',async t=>{const root=await fixture(t);await packageDownloads(root);const web=unzipSync(new Uint8Array(await readFile(join(root,'public/downloads/pixelwall-standalone.zip'))));assert.match(strFromU8(web['run-local.mjs']),/fileURLToPath\(new URL/);assert.ok(web['index.html']);assert.ok(web['README.txt']);const cli=unzipSync(new Uint8Array(await readFile(join(root,'public/downloads/pixelwall-cli.zip'))));assert.deepEqual(Object.keys(cli).sort(),['README.md','package.json','pixelwall.mjs']);});
