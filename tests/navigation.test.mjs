...
test('desktop second instance recreates a previously closed window',async t=>{
 ...
 const app=new EventEmitter();Object.assign(app,{setName(){},setPath(){},getVersion:()=>"0.2.0",getPath:()=>folder,isPackaged:false,requestSingleInstanceLock:()=>true,quit(){},whenReady:async()=>{}});
 ...
 globalThis.__pixelwallElectronMock={app,BrowserWindow,Menu:{buildFromTemplate:template=>template,setApplicationMenu(){}},protocol:{registerSchemesAsPrivileged(){},handle(){}},net:{},shell:{},session:{defaultSession:{setPermissionRequestHandler(){}}},dialog:{showErrorBox(title,message){throw Error(`${title}: ${message}`);}}};t.after(()=>delete globalThis.__pixelwallElectronMock);
 const source=(await readFile(new URL('../desktop/main.mjs',import.meta.url),'utf8')).replace("import { app, BrowserWindow, protocol, net, shell, session, dialog, Menu } from 'electron';","const { app, BrowserWindow, protocol, net, shell, session, dialog, Menu } = globalThis.__pixelwallElectronMock;");const path=join(folder,'main.mjs');await writeFile(path,source.replace("from './update-checker.mjs'",`from ${JSON.stringify(new URL('../desktop/update-checker.mjs',import.meta.url).href)}`));await import(pathToFileURL(path));await Promise.resolve();
