...
test('desktop second instance recreates a previously closed window',async t=>{
 ...
 globalThis.__pixelwallElectronMock.autoUpdater=new EventEmitter();
 globalThis.__pixelwallUpdateLibraryMock={autoUpdater:new EventEmitter()};t.after(()=>delete globalThis.__pixelwallUpdateLibraryMock);
 const source=(await readFile(new URL('../desktop/main.mjs',import.meta.url),'utf8'))
   .replace(/import \{([^}]+)\} from 'electron';/,(_match,names)=>`const {${names.replace(/\s+as\s+/g,': ')}} = globalThis.__pixelwallElectronMock;`)
   .replace("import electronUpdater from 'electron-updater';","const electronUpdater = globalThis.__pixelwallUpdateLibraryMock;");
 const path=join(folder,'main.mjs');await writeFile(path,source.replace(/from '(\.\/[^']+\.mjs)'/g,(_match,name)=>`from ${JSON.stringify(new URL('../desktop/'+name.slice(2),import.meta.url).href)}`));await import(pathToFileURL(path));await Promise.resolve();
