...
test('desktop second instance recreates a previously closed window',async t=>{
 ...
 const app=new EventEmitter();Object.assign(app,{setName(){},setPath(){},requestSingleInstanceLock:()=>true,quit(){},whenReady:async()=>{}});
 const windows=[];class BrowserWindow extends EventEmitter{constructor(){super();this.destroyed=false;this.webContents={setWindowOpenHandler(){},on(){}};windows.push(this);}static getAllWindows(){return windows.filter(window=>!window.destroyed);}loadURL(){return Promise.resolve();}isDestroyed(){return this.destroyed;}isMinimized(){return false;}show(){assert.equal(this.destroyed,false);}focus(){assert.equal(this.destroyed,false);}}
 globalThis.__pixelwallElectronMock={app,BrowserWindow,protocol:{registerSchemesAsPrivileged(){},handle(){}},net:{},shell:{},session:{defaultSession:{setPermissionRequestHandler(){}}},dialog:{showErrorBox(title,message){throw Error(`${title}: ${message}`);}}};t.after(()=>delete globalThis.__pixelwallElectronMock);
 const source=(await readFile(new URL('../desktop/main.mjs',import.meta.url),'utf8')).replace("import { app, BrowserWindow, protocol, net, shell, session, dialog } from 'electron';","const { app, BrowserWindow, protocol, net, shell, session, dialog } = globalThis.__pixelwallElectronMock;");const path=join(folder,'main.mjs');await writeFile(path,source);await import(pathToFileURL(path));await Promise.resolve();
