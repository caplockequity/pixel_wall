...
await prepareRuntimes(join(webOut,'runtimes'));
...
await prepareRuntimes(join(cliOut,'runtimes'));
...
const luaWorker = await rolldown({input:join(sourceRoot,'app/lua-node-worker.mjs'),platform:'node',external:/^node:/});
try { await luaWorker.write({file:join(cliOut,'lua-node-worker.mjs'),format:'es',sourcemap:false,banner:"import {createRequire as pixelwallWorkerRequire} from 'node:module'; import {fileURLToPath as pixelwallWorkerPath} from 'node:url'; const require=pixelwallWorkerRequire(import.meta.url); const __filename=pixelwallWorkerPath(import.meta.url);"}); } finally { await luaWorker.close(); }
...
await cp(join(sourceRoot, 'public/examples/agent-ninja-panda-commands.json'), join(cliOut, 'agent-ninja-panda-commands.json'));
